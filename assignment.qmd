...
# the %>% runs the next function to the previous
```
...
#use read_csv() to import a tibble not a regular data frame
...
#creates a new tibble with only with the selected cols
```
...
cereal_fiber <- cereal %>% 
  select(name, calories, fiber)
#created new data object with the selected cols
```
...
cereal %>% 
  select(-name, -mfr)
#creates a table with everything but the colms using the "-"
```
...
#created a new object with the selected cols: name, manufacurer and sugars
```
...
cereal_sugar %>% 
  rename(manufacturer = mfr,
         cereal_name = name)
#renamed the "name" col to "cereal_name"
```
...
#moved mfr to the left of the name col
```
...
cereal_sugar %>% 
  filter(sugars > 12)
#gives us a table with any row that has a sugar value greater than 12
```
...
#shows 23 cereals from kelloggs
```
...
cereal_sugar  %>% 
  filter(sugars > 12,
         mfr == "Kelloggs")
#to combine the filters, add a comma between arguments
# outputs a table with 4 rows
```
...
cereal %>% 
  ...
  filter(potass < 30)
#found that 8 cereals had potassium levels less than 30
#note that potassiiums col was in short had
...
cereal %>% 
  filter(calories > 120) %>% 
  #filters only cereals with 120 calories
  select(name, fiber, fat, sodium)
#then only displays the selected cols
# 8 cereals that meet these requirements
...
cereal %>% 
  arrange(calories)
#by defualt this sorts calories by lowest to highest (top down)
```
...
#the first col will have priority before caloriies, a->z on mfr THEN lowest to highest on calories
...
# multiplied by two to get the total sugars for TWO servings
...
#adds a new column with the sugars PLUS the sugars from milk
```
...
#string multiple mutations with commas
```
...
bison2 = bison %>% 
  mutate(age = rec_year - animal_yob,
         weight_kg = animal_weight * 0.452592)
```
...
A: Do female or male bison weigh more, and how does this look over their life cycle?
...
```{r}
library(ggplot2)
bison_plot = ggplot(data = bison2, aes(age, weight_kg),) + 
  geom_point(aes(shape= animal_sex,
                 fill= animal_sex))+
  scale_fill_manual(values = c("red", "blue") )+
  scale_shape_manual(values = c(21, 22)) +
  geom_smooth(aes(fill = animal_sex), method = "lm")
            
             
bison_plot
```

1\) we found that male bison gain weight throughout their lives at a much faster rate than females do, they also live much shorter lives.

2\) Why do male bison have shorter lives? are they harvested or are their deaths natural? a cause of death would be helpful
...
```{r}
ggsave("bison_plot.png", plot = bison_plot)
```
